  ...
  return (
    <View style={[styles.sidebarContainer, { backgroundColor: theme.surface, borderColor: theme.borderLight }]}>
      ...
      <View style={styles.sidebarFooter}>
        ...
        <TouchableOpacity
          style={[styles.sidebarItem, { backgroundColor: hoverToggle ? theme.surfaceSecondary : 'transparent' }]}
          onPress={toggleTheme}
          // @ts-ignore
...
export default function TabsLayout() {
  ...
  // Cambio de tu compañero: variables separadas para permisos (más legible)
  const canViewOwners = can(user, 'owners', 'view');
  const canViewPets = can(user, 'pets', 'view');
  const canViewTreatments = can(user, 'treatments', 'view');
  const canViewStore = can(user, 'store', 'view');
          ...
          {canViewTreatments && (
            <Tabs.Screen
              ...
              options={{
                title: 'Citas',
                tabBarIcon: ({ color, focused }) => (
                  <TabIcon name={focused ? 'medical' : 'medkit-outline'} color={color} focused={focused} />
                ),
              }}
            />
          )}
